...
async function withApi(testBody) {
  const dir = mkdtempSync(join(tmpdir(), 'cloth-products-filter-'));
  const dbPath = join(dir, 'cloth.sqlite');
  const port = 3860 + Math.floor(Math.random() * 500);
...
async function createProduct(port, overrides = {}) {
  const suffix = `${Date.now()}-${Math.random().toString(16).slice(2)}`;
  const { response, payload } = await requestJson(port, '/api/products', {
    method: 'POST',
...
async function getProducts(port, query) {
  const params = new URLSearchParams(query);
  const { response, payload } = await requestJson(port, `/api/products?${params.toString()}`);
...
test('products filtering supports market, category, price, search, sort, and pagination', async () => {
  await withApi(async (port) => {
    ...
    const uk = await getProducts(port, { market: 'UK', search: prefix, limit: '50' });
    ...
    const bagsInRange = await getProducts(port, {
      search: prefix,
      category: '包袋',
      minPrice: '1000',
      maxPrice: '3500',
    ...
    const page1 = await getProducts(port, { search: prefix, page: '1', limit: '3', sort: 'createdAt_asc' });
    const page2 = await getProducts(port, { search: prefix, page: '2', limit: '3', sort: 'createdAt_asc' });
...
test('products filtering rejects invalid query params with 400 JSON', async () => {
  await withApi(async (port) => {
    ...
    for (const [query, expectedError] of cases) {
      const { status, body } = await getProducts(port, query);
